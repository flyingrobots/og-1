...
\begin{tikzpicture}[node distance=0.85cm and 0.8cm]
  ...
  \node[ogbox, right=of proj, minimum width=2.2cm] (basis) {basis view\\\(\Enc_S(O(h))\)};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \draw[thick] (0,0) rectangle (13.8,6.5);
  \node[anchor=west] at (0.2,6.2) {\(\Delta_{\mathcal Q}\) (task-relevant pairs)};

  % Projection-separated set
  \draw[thick] (3.6,3.2) circle (2.2);
  \node at (3.6,4.9) {\(\mathcal S_{\proj}\)};
  \node[align=center] at (3.6,1.5) {\scriptsize separated at\\[-1pt]\scriptsize projection};

  % Basis-separated subset
  \draw[thick,dashed] (3.3,3.4) circle (1.2);
  \node at (3.3,3.4) {\(\mathcal S_{\basis}\)};

  % Accumulation-separated set
  \draw[thick] (7.8,3.2) circle (2.3);
  \node at (7.8,4.9) {\(\mathcal S_{\acc}\)};
  \node[align=center] at (7.8,1.5) {\scriptsize separated after\\[-1pt]\scriptsize accumulation};

  % Region note: recovered by accumulation
  \draw[->,thick] (10.8,3.2) -- (9.5,3.2);
  \node[anchor=west,align=left] at (10.8,3.2) {\scriptsize accumulation-only\\[-1pt]\scriptsize separations};
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=1cm and 0.8cm]
  ...
  % Bottom row (aligned with ends of the top row to span the diagram)
  \node[ogaltbox, below=1.5cm of h, minimum width=2.8cm] (t) {truth functional\\\(T_{\mathcal Q}(h)\)};
  \node[ogaltbox, below=1.5cm of a, minimum width=2.7cm] (d) {task decoder\\\(D':\widehat{\Tr}_S\to\mathcal Y_{\mathcal Q}\)};
  ...
  % Midway text placement guarantees no overlaps
  \draw[ogthin] (t) -- node[midway, above, align=center] {\scriptsize finite-\(X\) regime: \(A^{\acc}_{\mathcal Q,\mu}=1\)\\[-1pt]\scriptsize corresponds to exact decodability on \(X\)} (d);
\end{tikzpicture}
...
\[
  \Enc_S : \Tr_S \to \prod_{b\in\mathcal{B}_S}\cod(b),
  \qquad
  \Enc_S(t) := (\,b(t)\,)_{b\in\mathcal{B}_S}.
\]
\end{definition}

\begin{remark}
\label{rem:basis-encoding}
The map \(\Enc_S\) packages all native basis observables into a single observable. Two raw traces have the same basis encoding exactly when they are indistinguishable in the observer's native coordinates.
\end{remark}
...
\subsection{Epistemic Coordinates Used in Signatures (Preview)}
\label{subsec:signature-epistemic-preview}

To keep the exposition readable, signatures in this section use one coordinate from the structural core and one coordinate whose full admissibility setup is completed in Section~\ref{sec:epistemic-layer}.

The task-conditioned degeneracy coordinate is already defined in Section~\ref{subsec:task-conditioned-degeneracy}:
\[
  \mathsf{Deg}^{\task}_{\nu,\mathcal{Q}}(S)
  =
  H_\nu\!\bigl(T_{\mathcal{Q}}(H)\mid X_S^{\acc}\bigr).
...
For insufficiency, fix the loss-bundle components \((\nu_{\mathcal{Q}},\ell_{\mathcal{Q}})\) for each task family \(\mathcal{Q}\), and preview the coordinate by
\[
  \epsilon_{\mathrm{opt}}(S,\mathcal{Q})
  :=
  \inf_{D':\,\widehat{\Tr}_S\to\mathcal{Y}_{\mathcal{Q}}}
  \mathbb{E}_{h\sim \nu_{\mathcal{Q}}}
  \!\Bigl[
    \ell_{\mathcal{Q}}
    \bigl(
  ...
  \Bigr].
\]
Section~\ref{sec:epistemic-layer} then gives the full OG-I definition of admissible decoders, loss-model scope, and its relation to epistemic observers.
...
The signature tuple includes task-conditioned and insufficiency coordinates. Section~\ref{subsec:signature-epistemic-preview} gives the compact preview used here; the full epistemic formalization is intentionally deferred to Section~\ref{sec:epistemic-layer} so that readers can first absorb the structural and signature-level geometry.
\end{remark}
...
In OG-I, the structural observer tuple \(S=(O,\mathcal{B}_S,M_S,K_S,E_S)\) and the induced maps \(O,\widehat{O}_S,\Enc_S\) are observer-intrinsic objects. Most comparison quantities are relative coordinates: aperture depends on \((\mathcal{Q},\mu_{\mathcal{Q}})\), degeneracy depends on \(\nu\), insufficiency and bias depend on \((\mathcal{Q},\nu_{\mathcal{Q}},\ell_{\mathcal{Q}})\), and typed pairwise distances depend on chosen semantic comparison views and budgets \((\tau,m)\).
\end{remark}
...
Equivalently: any single-score ranking that is used as an exact replacement must fabricate at least one dominance claim that is not present in the underlying observer geometry.
\end{remark}
...
This yields a coordinate set beyond scalar distance: aperture, degeneracy, structural insufficiency \(\epsilon_{\mathrm{opt}}\), bias gap \(\BiasGap\), semantically typed pairwise coordinates, and a scoped budget-elasticity diagnostic. The separation theorems establish joint necessity of the core structural/epistemic coordinates (aperture, degeneracy, insufficiency, and signatures): state and provenance can separate, task sufficiency can separate from historical faithfulness, and accumulation can separate from projection and basis. The finite witnesses provide explicit constructions of these regimes, including an explicit elasticity witness for the scoped diagnostic.
